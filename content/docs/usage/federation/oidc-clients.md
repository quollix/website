...
Deleting an OIDC client immediately prevents that client from using Quollix as its OIDC provider. Regenerating credentials invalidates the existing OIDC client credentials. After regeneration, update the credentials in the client service so that users can sign in again.
